...
function PreRegistrationManagement() {
  ...
  useEffect(() => {
    ...
    setSettings({
      pre_registration_enabled: Boolean(
        selectedEvent.pre_registration_enabled,
      ),
      ...
      section_one_title:
        selectedEvent.registration_form_config?.section_one_title ||
        "Church and delegate information",
      section_two_title:
        selectedEvent.registration_form_config?.section_two_title ||
        "Payment details",
      custom_sections:
        selectedEvent.registration_form_config?.custom_sections || [],
    });
  }, [selectedEvent]);
  ...
  const save = async (submitEvent) => {
    ...
    try {
      ...
      await updateEventPreRegistration(selectedId, {
        ...
        pre_registration_slug: settings.pre_registration_enabled
          ? settings.pre_registration_slug || generatedSlug
          : null,
        ...
        registration_gcash_recipient_name:
          settings.registration_gcash_recipient_name || null,
        registration_gcash_number:
          settings.registration_gcash_number || null,
  ...
  if (loading)
    return <LoadingState label="Loading events for pre-registration..." />;

  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900">
          Event Pre-Registration Forms
        </h1>
        <p className="text-sm text-slate-500">
          Select an event, configure its registration form, and add optional
          user input sections.
        </p>
      </div>
      <ErrorState
      ...
      {events.length === 0 ? (
        <EmptyState label="Create an event first before configuring pre-registration." />
      ) : (
              ...
              {events.map((event) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`h-2.5 w-2.5 shrink-0 rounded-full ${event.pre_registration_enabled ? "bg-emerald-500" : "bg-slate-300"}`}
                    />
                  </div>
