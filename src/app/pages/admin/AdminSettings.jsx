...
function AdminSettings() {
  ...
  const toggle = (field, label) => <label className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 p-3 text-sm" key={field}>
    <span>{label}</span>
    <input type="checkbox" checked={settings[field]} onChange={(event) => update(field, event.target.checked)} />
  </label>;

  return <div className="space-y-5">
    <div>
      <h1 className="text-slate-900" style={{ fontSize: "22px", fontWeight: 700 }}>Settings</h1>
      <p className="text-slate-500 text-sm">System preferences, notifications, accessibility, and app information</p>
    </div>
    ...
    <section className="bg-white rounded-2xl p-5 border border-slate-100 space-y-3">
      ...
      {toggle("reviewNotifications", "Review queue notifications")}
    ...
    <section className="bg-white rounded-2xl p-5 border border-slate-100 text-sm text-slate-600">
      ...
      <p>NELPAC SYSTEM Admin Interface</p>
      <p>React + Vite + Supabase Auth, RLS, RPCs, Storage, and SQL views.</p>
      <p>No API keys are hardcoded. The app reads VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY.</p>
    </section>
  </div>;
}
