...
const actions = [
  ...
  { icon: Calendar, label: "Community", path: "/user/events" },
    ...
    <div className="space-y-5">
      ...
      >
        <h1 style={{ fontSize: "24px", fontWeight: 800 }}>
          Welcome,{" "}
          {getProfileDisplayName(profile, profile.email || "No name provided.")}
        </h1>
        ...
        <p className="mt-4" style={{ fontSize: "36px", fontWeight: 900 }}>
        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              key={action.path}
              to={action.path}
              className="bg-white rounded-2xl p-4 border border-slate-100 text-slate-700 hover:text-blue-700"
            >
        ...
        <section className="bg-white rounded-2xl p-5 border border-slate-100">
          ...
          {visibleEvents.slice(0, 5).length === 0 ? (
            ...
            visibleEvents.slice(0, 5).map((event) => (
              <div
                key={event.id}
                className="flex gap-3 border-b border-slate-50 py-2 text-sm"
              >
                {event.image_url ? (
                  <img
                    src={event.image_url}
                    alt={event.title}
                    className="h-12 w-16 rounded-lg object-contain bg-slate-100"
                  />
                ) : (
          ...
          {posts.slice(0, 5).length === 0 ? (
            ...
            posts.slice(0, 5).map((post) => (
              ...
              >
                ...
                <div>
                  <b>{post.title}</b>
                  <p className="text-slate-500">{post.category}</p>
                </div>
              </div>
            ))
          )}
      ...
      <section className="bg-white rounded-2xl p-5 border border-slate-100">
        ...
        {claims.filter((claim) => claim.user_id === user.id).length === 0 ? (
